...
# 실제값과 예측값을 비교한 그래프 그리기
plt.figure(figsize=(10, 6))
sns.set(style="whitegrid")

# 실제값과 예측값 시각화
plt.plot(y_val.values, label='Actual Count', color='blue', alpha=0.7)
plt.plot(val_pred, label='Predicted Count', color='red', alpha=0.7)

plt.title('Actual vs Predicted Counts (Validation Data)', fontsize=16)
plt.xlabel('Samples', fontsize=12)
plt.ylabel('Count', fontsize=12)
plt.legend()
plt.show()

# 실제값과 예측값의 차이를 분포로 나타내기 (잔차)
residuals = y_val - val_pred
plt.figure(figsize=(10, 6))
sns.histplot(residuals, bins=30, kde=True, color='green', stat='density', linewidth=0)
